...
def generate_module_packages_file(all_packages, output_path):
    ...
    # Sort by (os_major, module_name, stream) for readability and convert to sets
    sorted_packages = {key: set(sorted(packages)) for key, packages in all_packages.items()}
    sorted_dict = dict(sorted(sorted_packages.items(), key=lambda x: (x[0][1], x[0][0], x[0][2])))

    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # Use pformat to generate clean Python code
    dict_content = pformat(sorted_dict, width=100, sort_dicts=False)
...
Auto-generated by scripts/generate_module_packages.py on {timestamp}
"""

MODULE_PACKAGES = {dict_content}
''')
