...
def generate_module_packages_file(all_packages, output_path):
    ...
    sorted_items = sorted(sorted_packages.items(), key=lambda x: (x[0][1], x[0][0], x[0][2]))

    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # Generate Python code that matches ruff formatting
    lines = []
    for (module_name, os_major, stream), packages in sorted_items:
        key_repr = f'("{module_name}", {os_major}, "{stream}")'

        # If few packages, put them on one line
        if len(packages) <= 3:
